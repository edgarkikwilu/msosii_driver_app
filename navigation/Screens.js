...
//Stacks
import PublicAppStack from './DriverStacks/PublicAppStack';
import AuthenticatedAppStack from './DriverStacks/AuthenticatedAppStack';
        ...
        API.registerUser(data.name,data.email,data.password,data.phone,(responseJson)=>{
          console.log(JSON.stringify(responseJson));
           if(responseJson.status){
              ...
              User.logout(()=>{
                dispatch({ type: 'SIGN_OUT' })
                data.toasterror.current.show(Language.DriverCreated, 2000, () => {});
              })
